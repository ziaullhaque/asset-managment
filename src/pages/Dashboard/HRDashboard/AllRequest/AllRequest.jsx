...
const AllRequest = () => {
  ...
  const statusBadge = (s) => {
    ...
    if (s === "pending")
      return <span className={`${base} badge-warning`}>Pending</span>;
    if (s === "approved")
      return <span className={`${base} badge-success`}>Approved</span>;
    return <span className={`${base} badge-error`}>Rejected</span>;
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="pl-9 pr-4 py-2 rounded-lg w-full  bg-gray-50 border border-[#006d6f]/30 rounded-lg focus:ring-2 focus:ring-[#006d6f]/40 outline-none appearance-none text-gray-500"
            >
...
const Stat = ({ icon, label, value, color }) => (
  <div className={`rounded-xl p-4 border bg-${color}-50 border-${color}-200`}>
    <div className="flex items-center gap-3">
      <div
        className={`w-9 h-9 bg-${color}-500 rounded-lg flex items-center justify-center text-white`}
      >
        {icon}
      </div>
      <div>
